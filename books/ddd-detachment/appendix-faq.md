...
`Verified` の `profile` は `Optional<Profile>` ではなく、直接 `Profile` として持ちます。プロフィール未設定のデータは `Unverified` として読み込み、プロフィールが揃った時点で `Verified` に遷移する振る舞いを Behavior に書く設計になります。これにより、呼び出し側が「必須と言いながら null が来るかもしれない」と身構える必要がなくなります。
...
本書の立場は、**属性セットごとに小さな Behavior (ユースケース) を切る**、というものです。1 つの巨大な `updateSubscription` ではなく、`changeFrequency` / `changeNextDeliveryDate` のように、更新する属性セットごとにメソッドを分けます。
...
本書では **UseCase** と **ApplicationService** を同義で扱い、文脈に応じて両方の呼び方を使います。責務はクリーンアーキテクチャの UseCase / Interactor と同じです。
