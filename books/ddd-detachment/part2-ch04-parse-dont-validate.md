...
実際のアプリケーションでは、エラーメッセージを日本語・英語などに切り替えたいことが多いです。メッセージを Issue に埋め込んでしまうと、i18n のたびにデコーダ定義の書き換えが必要です。
...
エラー UX はデコーダ設計の一部です。エラーコード体系を早めに決め、クライアント・サーバー双方でその識別子を共有できるようにしておくと、後から i18n 対応を追加するときの修正が局所的に収まります。
