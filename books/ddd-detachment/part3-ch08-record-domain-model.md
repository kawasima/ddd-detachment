...
**ドメインモデル貧血症**（Martin Fowler 命名）は、「フィールドの Getter/Setter しか持たず、業務知識がドメインオブジェクトの外に散らばっている状態」を指します。Fowler の原定義では「振る舞いがなくデータだけを持つオブジェクト」というのが批判の核心であり、ビジネスロジックがドメインオブジェクトの外に置かれることを問題視しています。
